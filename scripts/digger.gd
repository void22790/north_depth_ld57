...
func _ready() -> void:
	timer.wait_time = Global.digger_time
...
func _on_timer_timeout() -> void:
	...
	var valid_ore_cells: Array[Vector2i] = []
	for cell in ore_cells:
		if ore_layer.get_cell_tile_data(cell) != null and not ice_layer.get_cell_tile_data(cell):
			valid_ore_cells.append(cell)
	
	if valid_ore_cells.is_empty():
		return
	
	var random_index = randi_range(0, valid_ore_cells.size() - 1)
	var target_pos = valid_ore_cells[random_index]
	
	if not ore_tile_health.has(target_pos):
	...
	ore_tile_health[target_pos] -= Global.digger_hit
